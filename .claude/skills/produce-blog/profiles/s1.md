...
Each post gets its own subfolder inside `blog-factory/output/<Topic>/`, named after the **blog's title** (not the CSV name, not a date), holding a single `index.md`. `<Topic>` is the same topic value going into the frontmatter below (the CSV's `topic` column) — create that topic subfolder if it doesn't already exist:

```
blog-factory/output/<Topic>/<title-slug>/index.md
```
...
```bash
mkdir -p "blog-factory/output/<Topic>/<title-slug>"
```

`mkdir -p` creates the `<Topic>/` folder alongside the post folder in one step if it isn't already there — no separate check needed.

If the post folder already exists (e.g. a prior run produced the same or a very similar title), don't overwrite it — append `-2`, `-3`, etc. to the slug until you find a free folder name. Check first:

```bash
ls "blog-factory/output/<Topic>/" 2>/dev/null | grep "^<title-slug>"
```
