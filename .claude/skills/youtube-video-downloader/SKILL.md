...
1. Read the JSON file passed as the argument (default: `video-injest/video.json`) to get the list of playlist URLs.
2. For each playlist URL, run a single `yt-dlp` pass against the playlist. yt-dlp expands the playlist and downloads every video in it:
   ...
   - Output each video into its own folder (named after the title) nested inside a single parent folder `video-injest/download-video`, with files inside named the same:
     ```
     --output "video-injest/download-video/%(title)s/%(title)s.%(ext)s"
     ```
   - This produces a structure like:
     ...
     └── Another Song/
         └── ...
     ```
3. All output goes into the `video-injest/download-video/` folder in the current working directory.
