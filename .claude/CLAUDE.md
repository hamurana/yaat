...
* **`topic` Is Inferred, Not Copied**: `extract-video-meta` writes the `topic`/`category` field as the literal sentinel `__INFER__` (YouTube's own category is useless). Replace it by **inferring from the categories already in use** in `Ark/Learning` — observed set: **Health, Investment, Habit, Charisma, Style**. Reuse an existing one before coining a new label.
...
* **Garbled Names vs. Uncertain Facts**: whisper routinely mangles well-known proper nouns (e.g. "Roy Balmeister" → **Roy Baumeister**, "Dan Erielli" → **Dan Ariely**, "encloved cognition" → **enclothed cognition**). Silently correcting an obvious, unambiguous misspelling of a famous name/term is fine — that is transcription cleanup, not invention. But never "correct" a *claim* (a statistic, date, study result); if the speaker states a fact that seems off, preserve it and flag inline, e.g. `[Transcript imprecise on the century]`.
