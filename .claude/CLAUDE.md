...
The `blog-factory/` folder is the second repurposing stage: it turns a curated batch of finished Ark notes into a single blog post. This is downstream of the whole `process-video` pipeline — it consumes the clean Section-1 summaries, it does **not** touch raw `download-video/` artifacts. Automated by the **`produce-blog` skill** (`.claude/skills/produce-blog/`); trigger with "produce a blog", "run the blog factory", or by pointing at a `.csv` in `blog-factory/`.
* **The manifest is a CSV, header `name,topic,title`.** Each row's `name` is an Ark note basename in `DD-MM-YYYY-N` form; `topic` is the Ark category; `title` is the original video title (context only — **the note is the source of truth**, not the title). One CSV = one blog. If several CSVs exist and the user didn't name one, ask which.
...
* **Find the synthesis angle; avoid "N tips from N videos."** Read all notes fully, then build one cohesive argument around the claim the batch collectively points to but no single video states outright. (Health example: insulin resistance is the silent throughline, and cheap daily movement beats the supplement economy.)
...
* **Output:** write to `blog-factory/<csv-basename>-blog.md` (e.g. `Health.csv` → `Health-blog.md`); re-running the same manifest **overwrites** it, by design. Start the file with YAML frontmatter (`title`, `topic`, `date`, and a `sources:` list of each `name` + its note's origin link) for traceability, then the post. Sign off by reporting the synthesis angle chosen, the word count, and any rows that failed to resolve.
